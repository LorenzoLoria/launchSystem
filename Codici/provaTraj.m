...
initialGuessY = linspace(x0(2), xf(2), nOptPoints+2) ; 
initialGuessY = initialGuessY(2:end-1) ; 
initialGuessZ = linspace(x0(3), xf(3), nOptPoints+2) ; 
...
%C = [earthRadius - min(distance) ; max(acc) - maxAcc ; max(steeringAngle) - maxSteeringAngle ; max(norm(thrust)) - maxThrust];
 C = [(earthRadius - min(distance))/earthRadius ; (max(acc) - maxAcc)/g0 ; (max(thrustMagnitude) - maxThrust)/maxThrust; (max(steeringAngle) - maxSteeringAngle)/maxSteeringAngle ];
