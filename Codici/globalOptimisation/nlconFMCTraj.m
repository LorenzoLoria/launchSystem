...
for i = 1:opt.nStages
    ...
    angleWrtVelMaxStage(i) = max(acosd(sum(ThrustIRF .* vel, 1)./ThrustIRFNorm./vNorm));
...
cin = [abs(angleWrtVelMax)-20;(accMax-6*mission.environment.g0)/mission.environment.g0] ; 
ceq = [ norm(stateCollocation(1:3,end,end) - mission.target.initialPointECI) ];
end